...
#include <cvx/util/misc/configuration.hpp>
...
void loadPoses(const PinholeCamera &cam, const CameraCalibration::Data &cdata,  vector<Affine3d> &target_to_sensor,
               vector<Affine3d> &gripper_to_base, double offset_x, double offset_y, double  offset_z )
{
...
void drawResiduals(const PinholeCamera &cam, const CameraCalibration::Data &cdata, const vector<Affine3d> &gripper_to_base, const Affine3d &sensor_to_base,
    ...
    for( uint i=0 ; i<cdata.image_paths_.size() ; i++ )  {
         ...
         const string path = "/tmp/new_hand_eye_smartsurg/" ;
...
int main(int argc, char *argv[]) {
    ...
    string data_folder, camera_intrinsics, output_file, launch_file, configuration_file ;
    bool compute_markers = false ;
    cv::Size pattern_grid(1, 1) ;
    float pattern_width = 0.062, pattern_gap = 0.01 ;
    bool print_help = false ;

    args.setDescription("Usage: hand_eye_registration [options]") ;
	...
	args.addOption("--launchFile", launch_file).required().setName("<launch_file>").setDescription("Path to launch file to store the TF tranformation publisher(ROS)") ;
	args.addOption("--configuration", configuration_file).required().setName("<configuration_file>").setDescription("Path to configuration file of the initial camera tranformation estimation") ;
    try {
	...
	CameraRobotConfiguration conf(configuration_file);
...
//     Matrix4d init ;  // REPLACED BY conf.initialCameraEstimation.matrix()
...
//     0, 0, 0, 1 ;
    ...
    Affine3d sensor_to_base(conf.initialCameraEstimation.matrix()) ;
   sensor_to_base = sensor_to_base.inverse() ;
    ...
    loadPoses(cam, cdata, target_to_sensor, gripper_to_base, conf.robotToTagX, conf.robotToTagY, conf.robotToTagZ) ;
   ...
   conf.setfinalCameraEstimation(sensor_to_base);
   conf.writeCameraTranformationToFile(launch_file);
