...
template <typename T, int D>
using container_t = std::vector<point_t<T, D>, Eigen::aligned_allocator<point_t<T, D>>> ;

template <class T, int D>
class PointList2: public container_t<T, D>
{
public:

    PointList2(uint n): data_(n, D) {}

    // fill with row major data i.e. (x, y) or column major ( x1, x2 ... y1, y2 ...)

    PointList2(T *data, int n, bool row_major = true) {
/*        if ( row_major )
            mat_ = Eigen::Map< Eigen::Matrix<T, Eigen::Dynamic, D, Eigen::RowMajor> >(data, n, D) ;
        else
            mat_ = Eigen::Map< Eigen::Matrix<T, Eigen::Dynamic, D, Eigen::ColMajor> >(data, n, D) ;
    ...
    PointList2(const PointList2<T, D> &other): data_(other) {
    }

    PointList2(const Eigen::Matrix<T, D, Eigen::Dynamic> &x): data_(x.cols()) {
    ...
    PointList2(const cv::Mat &src) {
         assert(src.cols == D);
         data_.resize(src.rows) ;
         cv::Mat dst(src.rows, src.cols, cv::DataType<T>::type, &data_[0], (size_t)(sizeof(T)));
         src.convertTo(dst, dst.type());
         assert( dst.data == (uchar*)data_.data());
    }

    point_t<T, D> center() const {
        return point_t<T, D>() ;
    }

    void axes(double &l1, point_t<T, D> &v1, double &l2, point_t<T, D> &v2) const ;
...
*/
    std::pair< point_t<T, D>, point_t<T, D> > bbox() const {
        //return std::make_pair(mat_.colwise().minCoeff(), mat_.colwise().maxCoeff()) ;
    }
